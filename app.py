...
def _build_name_map(daily, patient):
    """차트번호(clean_no 정규화) → 환자이름 맵 — 일마 성명 우선, 차트 이름 보완.

    suspects/3way테이블/AI텍스트가 공유한다 — 세 곳의 이름 표기가 항상 일치하도록
    한 곳에서만 정의(중복 구현 시 키 정규화가 어긋나 이름 누락되는 사고 방지)."""
    name_map = {}
    if daily is not None and not daily.empty \
            and "차트번호" in daily.columns and "성명" in daily.columns:
    ...
    if patient is not None and not patient.empty \
            and "차트번호" in patient.columns and "이름" in patient.columns:
...
def find_channel_suspects(channel, hansol, daily, patient, totals=None, top_n=12,
    ...
    # 키는 clean_no 정규화 — 파싱 시 차트번호가 이미 clean_no로 정규화돼 있어
    # 아래 name_map.get(str(ch).strip()) 조회와 동일하게 매칭된다.
    name_map = _build_name_map(daily, patient)
...
_EMPTY_METHODS = {"카드": 0, "현금": 0, "이체": 0, "플랫폼": 0}


def _patient_channel_diffs(p_pivot, d_pivot):
    """채널별 환자단위 (일마-차트) 차이 목록 {채널: [(차트#, diff), ...]} — diff 0 제외.

    채널 정의는 compute_channel_recon과 동일(카드 / 현금+이체 / 플랫폼)해야
    [오차요약]의 검산·후보 탐색이 채널대사 수치와 어긋나지 않는다."""
    diffs = {"카드": [], "현금+이체": [], "플랫폼": []}
    for ch in set(p_pivot) | set(d_pivot):
        p = p_pivot.get(ch, _EMPTY_METHODS)
        d = d_pivot.get(ch, _EMPTY_METHODS)
        for chan, v in (
            ("카드", d["카드"] - p["카드"]),
            ("현금+이체", (d["현금"] + d["이체"]) - (p["현금"] + p["이체"])),
            ("플랫폼", d["플랫폼"] - p["플랫폼"]),
        ):
            if v != 0:
                diffs[chan].append((ch, v))
    return diffs


def _find_exact_combos(diff_items, gap, max_items=20, max_combos=3):
    """합계가 gap과 정확히 일치하는 2~3건 조합 탐색 (복합 원인 후보).

    |금액| 상위 max_items 건만 대상으로 해 탐색량을 유계로 유지한다
    (C(20,2)+C(20,3)=1,330회 — 결정론적·즉시 완료). 반환은 최대 max_combos개."""
    items = sorted(diff_items, key=lambda x: -abs(x[1]))[:max_items]
    found = []
    for r in (2, 3):
        for combo in combinations(items, r):
            if sum(v for _, v in combo) == gap:
                found.append(combo)
...
def build_gap_summary_lines(channel_df, p_pivot, d_pivot, name_map, h_um, d_um):
    """[오차요약] 섹션 — '찾아야 할 금액'을 차트(EMR) 기준으로 서술 + 유력 후보 힌트.

    AI 추론에 맡기지 않고 코드가 확정 계산하는 부분:
      ① 채널별 '구글 일일마감/단말기 마감이 베가스 마감보다 ±X원 많음/적음'
         (부호·금액 창작 방지 — 용어는 정식 명칭: 한솔=단말기 마감·일마=구글
         일일마감·차트=베가스 마감)
      ② 채널 차이금액과 정확히 일치하는 환자 1명(단일일치후보) — 단일 원인 판정 근거
      ③ 단일 일치가 없으면 2~3건 조합 일치 후보 — 복합 원인 판정 근거
      ④ 검산: 환자별 차이 합 = 채널 차이인지, 미특정 잔여가 얼마인지
      ⑤ 카드 한-일 차이와 정확히 일치하는 PG-only/front-only 건 — 오류 위치
         (한솔에 없음 = 미수납/수단 오기재 의심, 일마에 없음 = 일마 기입누락 의심)
    """
    def _iv(v):
        try:
    ...
    def _cmp(subject, v):
        """subject는 조사 포함('구글 일일마감이' 등). v>0 = 베가스 마감보다 많음(차트 기준 서술)."""
        if v is None or v == 0:
            return None
        word = "많음" if v > 0 else "적음"
        return f"{subject} 베가스 마감보다 {abs(v):,}원 {word}({v:+,})"

    def _pat(ch):
        nm = (name_map.get(ch, "") or "?")[:10].replace("|", " ")
        return f"{nm}(차트#{ch}){_pivot_presence_tag(ch, p_pivot, d_pivot)}"

    diffs = _patient_channel_diffs(p_pivot, d_pivot)
    L = ["\n[오차요약 — 베가스 마감(차트) 기준·코드 확정계산: 찾아야 할 금액과 유력 후보]"]
    for _, r in channel_df.iterrows():
        chan = r["채널"]
        dj = _iv(r.get("일마-차트"))
        hj = _iv(r.get("한솔-차트"))
        hd = _iv(r.get("한솔-일마"))
        items = diffs.get(chan, [])
        parts = [x for x in (_cmp("구글 일일마감이", dj), _cmp("단말기 마감이", hj)) if x]
        if not parts and not items:
            L.append(f"{chan}: 차이 없음")
            continue
        L.append(f"{chan}: " + (" · ".join(parts) if parts else "합계 차이 없음"))

        if dj:
            singles = [(c, v) for c, v in items if v == dj]
            if len(singles) == 1:
                c, v = singles[0]
                L.append(f"  ↳단일일치후보(유일): {_pat(c)} 일-차 {v:+,} — "
                         "채널 차이금액과 정확히 일치(단일원인 유력)")
            elif singles:
                names = " / ".join(_pat(c) for c, _ in singles[:3])
                extra = f" 외 {len(singles) - 3}명" if len(singles) > 3 else ""
                L.append(f"  ↳동일금액 일치후보 {len(singles)}명: {names}{extra} — "
                         "단독 확정 불가·각각 검토")
            else:
                for combo in _find_exact_combos(items, dj):
                    expr = " + ".join(f"{_pat(c)}{v:+,}" for c, v in combo)
                    L.append(f"  ↳조합일치후보({len(combo)}건 복합): {expr} = {dj:+,}")
            total = sum(v for _, v in items)
            resid = dj - total
            if resid == 0 and items:
                L.append(f"  ↳검산: 환자별 차이 {len(items)}건 합계 = 일-차 {dj:+,} "
                         "(전액 환자단위 분해·잔여 0)")
            else:
                L.append(f"  ↳검산: 환자별 차이 합계 {total:+,} vs 일-차 {dj:+,} → "
                         f"미특정 잔여 {resid:+,}원 (환자단위 분해 밖 — PG-only/환불/미링크 행 검토)")
        elif items:
            L.append(f"  ↳합계는 일치하나 환자별 불일치 {len(items)}건이 +/-로 상쇄 — 각각 검토 필요")

        # 카드 채널: 한-일 차이는 환자단위(일-차) 분해 밖 — PG-only/front-only에서 원인 탐색
        if chan == "카드" and hd:
            if hd > 0 and h_um is not None and not h_um.empty:
                m = [x for x in h_um.to_dict("records") if _safe_int(x.get("금액", 0)) == hd]
                if len(m) == 1:
                    x = m[0]
                    L.append(f"  ↳한-일 단일일치: 한솔 PG-only {str(x.get('시간표시', ''))[:8]} "
                             f"{hd:,}원(승인{str(x.get('승인번호', ''))[-8:]}) — 구글 일일마감에 없음: "
                             "구글 일일마감 기입누락 또는 결제수단 오기재 의심")
                elif m:
                    L.append(f"  ↳한-일 동일금액 한솔 PG-only {len(m)}건({hd:,}원) — "
                             "구글 일일마감 누락/수단 오기재 후보·각각 검토")
            elif hd < 0 and d_um is not None and not d_um.empty:
                m = [x for x in d_um.to_dict("records") if _safe_int(x.get("카드", 0)) == -hd]
                if len(m) == 1:
                    x = m[0]
                    nm = str(x.get("성명", ""))[:10]
                    L.append(f"  ↳한-일 단일일치: 일마 front-only {nm}"
                             f"(차트#{clean_no(x.get('차트번호', ''))}) {-hd:,}원 — 단말기 마감(PG)에 없음: "
                             "실제 결제를 안 받았거나(미수납) 현금/이체를 카드로 오기재 의심")
                elif m:
                    L.append(f"  ↳한-일 동일금액 일마 front-only {len(m)}건({-hd:,}원) — "
                             "미수납/수단 오기재 후보·각각 검토")
    return L
...
def build_ai_text(hansol, daily, daily_refund, patient, channel_df,
    ...
    섹션 구성:
      ...
      [오차요약]           — 차트(EMR) 기준 '찾아야 할 금액' 확정 서술 + 채널 차이와
                            정확 일치하는 단일 환자/2~3건 조합 후보 + 검산 (코드 확정계산
                            → AI의 단일 원인/복합 원인 분기 판정 근거)
    ...
    # ── 차트번호 → 이름 맵 (오차요약·3-way 통합·원시거래행 공용) ──
    name_map = _build_name_map(daily, patient)

    # ── [오차요약 — 차트 기준 확정 서술 + 단일/조합 일치 후보 + 검산] ──
    # '찾아야 할 금액'과 유력 후보를 코드가 먼저 확정해 AI의 부호·금액 창작을 차단하고
    # 단일 원인/복합 원인 분기 판정의 근거를 제공한다.
    if has_nonzero or has_patient_diff:
        L.extend(build_gap_summary_lines(channel_df, p_pivot, d_pivot, name_map, h_um, d_um))
...
AI_SYSTEM = (
    "병원 정산 분석관. ★용어 정의(정확히 사용)★: "
    "한솔=단말기 마감(PG 카드승인내역) · 일마=구글 일일마감(프론트 수기 마감기록) · "
    "차트=베가스 마감(EMR 환자별집계·세무 기준원장). 입력에 나오는 '차트마감'은 베가스 마감, "
    "'일일마감'은 구글 일일마감과 같은 파일이다. "
    "입력 데이터는 토큰 절약을 위해 축약 표기(한솔/일마/차트, 한-차/한-일/일-차)를 쓰지만, "
    "출력 보고서에서는 반드시 정식 명칭 '단말기 마감'·'구글 일일마감'·'베가스 마감'으로 표기한다"
    "(예: '구글 일일마감이 베가스 마감보다 5,000원 적음'). "
    "입력은 이 3개 파일을 차트번호·승인번호로 join한 raw cross-file 구조다. "
    ...
    "안 보이는 환자쌍·소액 불일치도 각각 독립 건으로 반드시 나열한다"
    "(채널 차이를 설명하지 않는 건은 '기타 오류건·참고사항' 섹션에).\n"
    ...
    "오류건으로 차이금액 집계에 포함하지 않고 '기타 오류건·참고사항'에서 안내만 한다.\n"
    ...
    "[R16] 최종 목표: 사용자가 ①'얼마를 찾아야 하는지'(오차 요약) ②'그 원인이 누구·무엇인지'를 "
    "즉시 알게 하는 것. 출력 첫 섹션은 반드시 '오차 요약' — 채널별로 베가스 마감(차트) 기준 "
    "'구글 일일마감/단말기 마감이 베가스 마감보다 X원 더 많다/적다'를 입력 [오차요약]·[채널대사] "
    "수치 그대로 서술한다(재계산·근사·부호 창작 금지). 원인 제시는 단일/복합 분기:\n"
    "  (a) 단일 원인 — 오류 건 1건의 차이금액이 채널 차이금액과 정확히 일치하고 근거가 확정 "
    "등급(승인번호확정·데이터검증·한솔판정·원시거래행 재계산·[오차요약] '단일일치후보(유일)')이면 "
    "그 1건만 '오차 원인'으로 제시한다 — TOP 목록을 만들지 않는다(매번 TOP3를 채우지 말 것). "
    "나머지 의심 건은 전부 '기타 오류건·참고사항'으로 분리하되 버리지 않는다(R2).\n"
    "  (b) 복합 원인 — 단일 건으로 설명되지 않고 2건 이상의 조합으로 나타나면([오차요약] "
    "'조합일치후보'·검산 잔여 참고) 의심 원인을 확률 높은 순 TOP3~5로 전부 제시한다. "
    ...
    "단일·복합 모두 각 원인에 반드시 3요소를 붙인다: ①베가스 마감 기준 부호·금액('구글 "
    "일일마감이 베가스 마감보다 +X원 많음') ②오류 위치 추정 — 어느 파일의 무엇이 문제인지"
    "(단말기 마감에 승인 없음 → 실제 결제를 안 받았거나(미수납) 결제수단 오기재 / 구글 "
    "일일마감에 없음 → 구글 일일마감 기입누락 / 베가스 마감에 없음 → 베가스 마감 기입누락"
    "(세무위험) / 두 금액이 오타패턴 → 금액 오타) ③확인법 — 어느 파일의 어느 환자·어느 "
    "내역을 열어 무엇을 보면 1분 내 판정되는지 1줄. 이 중요도 별(★~★★★)은 원인 확률 표시로, "
    "검토대상 단서등급(★★=승인번호확정, ★=금액·gap매칭)과는 다른 축이다 — 혼동 금지. "
    "출력 토큰이 부족하면 하위 섹션을 압축하되 오차 요약·오차 원인·검산은 반드시 완결한다."
)
...
STEP0. [데이터검증] 확정 오입력 우선 처리 (섹션이 있으면 절대 먼저)
  ...
  · 유형B(차트번호오타): 이름·금액 동일·번호만 상이. 차트마감(EMR)은 차트번호 오입력이
    ...
    차이금액 집계에 포함하지 않고 '기타 오류건·참고사항'에서 안내만(합계 영향 없음 명시).
  ...
  · 이 섹션의 모든 건은 출력의 '오차 원인'(채널 차이를 설명하는 건) 또는
    '기타 오류건·참고사항'(합계 영향 없는 건 — 유형B 등)에 누락 없이 배치한다.

STEP1. [채널대사]·[오차요약] 차이값 확정
  · 채널별 한-차/한-일/일-차 차이를 정확히 메모 (부호 포함).
  · [오차요약] 섹션은 코드가 확정 계산한 값이다 — 차트 기준 서술(누가 차트보다 얼마 많은지),
    단일일치후보/조합일치후보, 검산 잔여를 그대로 신뢰하고 출력의 기준 수치로 사용한다.
...
STEP3. [차트번호별 3-way 통합]의 차이행을 환자 단위로 빠짐없이 검토
  ...
  · ★중요★ 채널 합계차이를 큰 환자 한 명으로 설명했어도 나머지 차이행을 생략하지 말 것.
    +/-로 상쇄되는 환자쌍(예: A −27,500 · B +27,500, 합계 0)도 각각 나열한다
    (채널 차이를 설명하지 않는 건은 출력의 '기타 오류건·참고사항'으로).
...
STEP6. 정합성 검산 (R10 — 출력에 결과를 반드시 포함)
  · 환자별 차이를 채널별로 합산해 [채널대사] 차이값과 대조: '채널차이 = Σ건별 차이'가 성립하는지 검산.
    [오차요약]의 '검산' 줄(코드 계산)이 있으면 그 잔여값과 일치해야 한다 — 다르면 내 계산을 재점검.
  · 성립하면 "검산 일치(잔여 0원)"와 합계차이를 주로 만든 '주요원인' 건을 명시.
  · 성립하지 않으면 "미특정 잔여 ±X원"을 명시하고 PG-only/front-only/환불 행에서 후보를 재탐색.
  · 단, 합계엔 안 잡히는 상쇄쌍·소액 불일치·유형C3도 버리지 말고 '기타 오류건·참고사항'에 남길 것
    (합계 일치는 생략 사유 아님).
    유형B(이름·금액 일치·번호만 오타)는 R12에 따라 '기타 오류건·참고사항'에서 번호 정정 안내만 한다.

STEP7. 오차 요약 확정 → 단일/복합 원인 판정 (R16 — 최종 목표)
  · 먼저 채널별 차이를 베가스 마감 기준 문장("구글 일일마감/단말기 마감이 베가스 마감보다
    X원 더 많다/적다")으로 확정한다 — [오차요약] 수치를 그대로 인용(재계산 금지).
    이것이 '이번에 찾아야 할 금액'이다.
  · 단일 판정: STEP0~6에서 확인한 오류 건 중 1건의 차이금액이 채널 차이금액과 1원 단위로
    정확히 일치하고 근거가 확정 등급(승인번호확정·데이터검증·한솔판정·원시거래행 재계산·
    [오차요약] '단일일치후보(유일)')이면 → 그 1건만 '오차 원인'으로 제시하고 TOP 목록은 생략.
    나머지 의심 건은 전부 '기타 오류건·참고사항'으로 분리한다(버리지 말 것 — R2).
  · 복합 판정: 단일 건으로 설명되지 않으면([오차요약] '조합일치후보'·검산 잔여 참고)
    채널 차이를 구성하는 오류 건들을 '원인' 단위로 묶어(같은 환자·같은 메커니즘·같은 패턴의
    다건 = 원인 1개, 건수 명시) 확률 높은 순 TOP3~5로 전부 제시한다.
    ①설명금액 ②독립 근거 수 ③근거 등급으로 ★★★/★★/★ 부여(기준은 R16) —
    후보가 3개 미만이면 있는 만큼만, ★ 정황 후보로 억지로 채우지 않는다.
  · 단일·복합 모두 각 원인에 3요소 필수: ①베가스 마감 기준 부호·금액 ②오류 위치 추정
    (어느 파일에서 난 오류인지 — 단말기 마감에 없음=미수납/수단 오기재 의심, 구글 일일마감에
    없음=구글 일일마감 기입누락, 베가스 마감에 없음=베가스 마감 기입누락(세무위험),
    오타패턴=금액 오타) ③확인법(1분 내 판정법) 1줄.
  · 마지막에 커버리지 1줄: '원인들로 채널 차이 ±X원 중 ±Y원 설명(잔여 ±Z원)'.

[출력 형식 — 900토큰 이내(반드시 끝까지 완결), 마크다운]

### 📌 오차 요약 (베가스 마감 기준 — 이번에 찾아야 할 금액)
- **{{채널}}**: 구글 일일마감이 베가스 마감보다 **{{X}}원 더 {{많다/적다}}**(일-차 {{±X}}){{한솔 있으면 " · 단말기 마감은 베가스 마감보다 {{Y}}원 {{많다/적다}}(한-차 ±Y)"}}
- {{차이 없는 채널}}: 차이 없음
([오차요약]·[채널대사] 수치 그대로 — 재계산·근사 금지)

### 🎯 오차 원인
◇ 단일 원인이 거의 확실한 경우(1건 = 채널 차이 정확일치 + 확정 근거) — 그 1건만:
- **{{환자명}}(차트#{{ch}}) — {{메커니즘 한 줄}}**: {{구글 일일마감/단말기 마감}}이 베가스 마감보다 {{±X}}원 {{많음/적음}} → 채널 차이 {{±X}}원 전액 설명(잔여 0원)
  - 오류 위치 추정: {{예: 단말기 마감에 승인 없음 → 결제를 안 받았거나(미수납) 결제수단 오기재 / 구글 일일마감 기입누락 / 베가스 마감 기입누락(세무위험) / 금액 오타}}
  - 근거: {{행·단서 인용(승인말미/시각/한솔판정/검증유형)}} / 확인법: {{어느 파일 어느 내역을 열어 무엇을 보면 되는지}}
◇ 복합 원인(2건 이상 조합)인 경우만 확률 높은 순 TOP3~5:
1. **★★★ {{원인 한 줄 — 메커니즘}}** — {{환자명}}(차트#{{ch}}) · {{채널}}: {{구글 일일마감/단말기 마감}}이 베가스 마감보다 {{±금액}}원 {{많음/적음}}{{· 다건이면 N건 합계}}
   오류 위치 추정: {{…}} / 근거: {{…}} / 확인법: {{…}}
2. **★★ …** 3. **★ …** (있는 만큼만 — 억지로 채우지 않음)
(마지막 줄 커버리지: "위 원인으로 {{채널}} 차이 {{±X}}원 중 {{±Y}}원 설명 · 잔여 {{±Z}}원{{잔여 있으면 재점검 안내}}")

### 📎 기타 오류건 · 참고사항 (채널 차이와 무관한 건 전부 — 해당 시에만, 각 1줄)
- 유형B 차트번호오타: {{환자}}(차트마감#{{A}} ↔ 일마기재 {{B}}) {{금액}}원 → 구글 일일마감 번호만 정정(합계 영향 없음)
- 유형C1/C2/C3 중 채널 차이와 무관한 건: {{환자}}(차트#) {{±금액}} · [{{위치}}] → {{검토 방향}}
- 상쇄쌍·소액 불일치·오타의심쌍(R11): {{환자}}(차트#) {{±금액}} → {{검토 방향}}
(0건이면 섹션 생략. '오차 원인'에 안 들어간 의심 건은 여기에 빠짐없이 — R0·R2)

### 🧮 검산 (R10)
- {{채널}}: 차이 {{±X}}원 = {{건별 차이 합산식}} → {{일치(잔여 0원) / 미특정 잔여 ±Y원 — PG-only/환불행 재점검}}
(차이 있는 채널만)

### 결론 (1~2문장)
가장 먼저 확인할 1건(환자명·차트#·어느 파일·베가스 마감 기준 ±금액)과 검산 결과(잔여 0원 여부)만 요약.
...
def main():
    ...
    st.caption("★ 결제채널별 파일 합계 차이를 먼저 산출 → 차이를 설명할 후보 거래 추적 | "
               "일마+차트 2개 또는 한솔+일마+차트 3개 분석 가능  \n"
               "용어: **한솔 = 단말기 마감(PG)** · **일마 = 구글 일일마감** · **차트 = 베가스 마감(EMR)**")
