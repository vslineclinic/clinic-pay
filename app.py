...
def build_gap_summary_lines(channel_df, p_pivot, d_pivot, name_map, h_um, d_um):
    ...
    AI 추론에 맡기지 않고 코드가 확정 계산하는 부분:
      ① 채널별 '일마/한솔이 차트보다 ±X원 많음/적음' (부호·금액 창작 방지)
    ...
    def _cmp(subject, v):
        """subject는 조사 포함('일마가'/'한솔이'). v>0 = 차트보다 많음(차트 기준 서술)."""
        if v is None or v == 0:
            return None
        word = "많음" if v > 0 else "적음"
        return f"{subject} 차트보다 {abs(v):,}원 {word}({v:+,})"
    ...
    L = ["\n[오차요약 — 차트(EMR) 기준·코드 확정계산: 찾아야 할 금액과 유력 후보]"]
    for _, r in channel_df.iterrows():
        ...
        parts = [x for x in (_cmp("일마가", dj), _cmp("한솔이", hj)) if x]
            ...
            if hd > 0 and h_um is not None and not h_um.empty:
                ...
                if len(m) == 1:
                    ...
                    L.append(f"  ↳한-일 단일일치: 한솔 PG-only {str(x.get('시간표시', ''))[:8]} "
                             f"{hd:,}원(승인{str(x.get('승인번호', ''))[-8:]}) — 일마에 없음: "
                             "일마 기입누락 또는 결제수단 오기재 의심")
                elif m:
                    L.append(f"  ↳한-일 동일금액 한솔 PG-only {len(m)}건({hd:,}원) — "
                             "일마 누락/수단 오기재 후보·각각 검토")
                ...
                if len(m) == 1:
                    ...
                    L.append(f"  ↳한-일 단일일치: 일마 front-only {nm}"
                             f"(차트#{clean_no(x.get('차트번호', ''))}) {-hd:,}원 — 한솔(PG)에 없음: "
...
AI_SYSTEM = (
    ...
    "즉시 알게 하는 것. 출력 첫 섹션은 반드시 '오차 요약' — 채널별로 차트(EMR) 기준 "
    "'일마/한솔이 차트보다 X원 더 많다/적다'를 입력 [오차요약]·[채널대사] 수치 그대로 서술한다"
    "(재계산·근사·부호 창작 금지). 원인 제시는 단일/복합 분기:\n"
    ...
    "단일·복합 모두 각 원인에 반드시 3요소를 붙인다: ①차트 기준 부호·금액('일마가 차트보다 "
    "+X원 많음') ②오류 위치 추정 — 어느 파일의 무엇이 문제인지(한솔에 승인 없음 → 실제 결제를 "
    "안 받았거나(미수납) 결제수단 오기재 / 일마에 없음 → 일마 기입누락 / 차트에 없음 → 차트 "
    "기입누락(세무위험) / 두 금액이 오타패턴 → 금액 오타) ③확인법 — 어느 파일의 어느 환자·어느 "
...
STEP7. 오차 요약 확정 → 단일/복합 원인 판정 (R16 — 최종 목표)
  · 먼저 채널별 차이를 차트 기준 문장("일마/한솔이 차트보다 X원 더 많다/적다")으로 확정한다 —
    [오차요약] 수치를 그대로 인용(재계산 금지). 이것이 '이번에 찾아야 할 금액'이다.
  ...
  · 단일·복합 모두 각 원인에 3요소 필수: ①차트 기준 부호·금액 ②오류 위치 추정(어느 파일에서
    난 오류인지 — 한솔에 없음=미수납/수단 오기재 의심, 일마에 없음=일마 기입누락, 차트에
    없음=차트 기입누락(세무위험), 오타패턴=금액 오타) ③확인법(1분 내 판정법) 1줄.
  · 마지막에 커버리지 1줄: '원인들로 채널 차이 ±X원 중 ±Y원 설명(잔여 ±Z원)'.

[출력 형식 — 900토큰 이내(반드시 끝까지 완결), 마크다운]

### 📌 오차 요약 (차트 기준 — 이번에 찾아야 할 금액)
- **{{채널}}**: 일마가 차트보다 **{{X}}원 더 {{많다/적다}}**(일-차 {{±X}}){{한솔 있으면 " · 한솔은 차트보다 {{Y}}원 {{많다/적다}}(한-차 ±Y)"}}
- {{차이 없는 채널}}: 차이 없음
([오차요약]·[채널대사] 수치 그대로 — 재계산·근사 금지)

### 🎯 오차 원인
◇ 단일 원인이 거의 확실한 경우(1건 = 채널 차이 정확일치 + 확정 근거) — 그 1건만:
- **{{환자명}}(차트#{{ch}}) — {{메커니즘 한 줄}}**: {{일마/한솔}}이 차트보다 {{±X}}원 {{많음/적음}} → 채널 차이 {{±X}}원 전액 설명(잔여 0원)
  - 오류 위치 추정: {{예: 한솔에 승인 없음 → 결제를 안 받았거나(미수납) 결제수단 오기재 / 일마 기입누락 / 차트 기입누락(세무위험) / 금액 오타}}
  - 근거: {{행·단서 인용(승인말미/시각/한솔판정/검증유형)}} / 확인법: {{어느 파일 어느 내역을 열어 무엇을 보면 되는지}}
◇ 복합 원인(2건 이상 조합)인 경우만 확률 높은 순 TOP3~5:
1. **★★★ {{원인 한 줄 — 메커니즘}}** — {{환자명}}(차트#{{ch}}) · {{채널}}: {{일마/한솔}}이 차트보다 {{±금액}}원 {{많음/적음}}{{· 다건이면 N건 합계}}
   오류 위치 추정: {{…}} / 근거: {{…}} / 확인법: {{…}}
...
### 결론 (1~2문장)
가장 먼저 확인할 1건(환자명·차트#·어느 파일·차트 기준 ±금액)과 검산 결과(잔여 0원 여부)만 요약.
