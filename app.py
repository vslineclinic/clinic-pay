...
AI_SYSTEM = (
    "병원 정산 분석관. 입력은 한솔(PG)·일마(프론트)·차트(EMR) 3개 파일을 "
    "차트번호·승인번호로 join한 raw cross-file 구조다. "
    "사전 요약이 아니라 추적 단서이므로, 직접 추론하여 어느 파일의 어느 거래(환자명·차트번호·금액)가 "
    "잘못됐는지 특정하라. "
    "[우선순위 절대규칙] ★★(승인번호로 동일환자 확정) > ★(금액·gap 매칭) > 일반. "
    "이 순서는 어떤 경우에도 뒤집을 수 없으며, 동일등급 내에서는 금액크기·gap일치도로 정렬."
)

AI_USER = """병원 정산 데이터 (3개 파일을 차트번호·승인번호로 통합):

{data}

[분석 지침 — 위 데이터는 사전요약이 아닌 raw 통합구조다]
1) [채널대사]에서 차이≠0 채널을 진단대상으로 식별
2) [★★승인번호확정매칭]은 PG↔EMR 승인번호로 환자가 이미 확정된 단서 — 절대 1순위 수정대상
3) [차트번호별 3-way 통합]에서 환자별 결제수단 오기재 패턴 추적:
   · 차트(28000/0/0/0) · 일마(0/28000/0/0) → 카드↔현금 오기재 (한 환자가 채널을 바꿔 적힘)
   · 차트(56000/...) · 일마(28000/...) · 한솔카드(28000) → 차트 중복기재 의심
   · 차트(0/0/0/0) · 일마(50000/0/0/0) → 차트에 결제건 누락
4) [한솔 PG-only]의 '동일금액일마환자' hint로 누락된 환자 추정
5) [일마 front-only]는 PG승인내역이 없음 → 결제수단 오기재 또는 미수납 가능성
6) [환불/취소] 행은 합계차이 원인일 수 있음 — 채널대사와 부호 검증
...
- **{{채널}}**: 한솔-차트=?원 / 한솔-일마=?원 / 일마-차트=?원
  - 원인 추정: (환자명·차트번호·금액 구체 인용)
  - 수정대상 TOP5 (★★ → ★ → 일반 순서 절대 유지):
    1. 환자명(차트#): "X파일의 Y원을 Z원으로 수정" / "X파일에 Y원 결제건 추가" 식으로 명확히

### 결론 (1~2문장)
어느 파일의 어느 환자(차트#·금액)를 어떻게 수정하면 모든 채널 합계가 맞는지."""
